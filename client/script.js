...
const changeHelloBtn = (e) => {sayHelloButton.classList.add('helloButtonBackground')}
...
const helloBtnOriginal = (e) => {sayHelloButton.classList.add('helloButtonOriginal')}
sayHelloButton.addEventListener('mouseout', helloBtnOriginal)
...
const ohMy = () => {
    axios.get('http://localhost:3000/animals').then(response => {
        for (let i = 0; i < response.data.length; i++){
            ...
            let paraDiv = document.querySelector("#appendP");
...
const repeatMyParam = () => {
    axios.get('http://localhost:3000/repeat/Bro').then(response =>{console.log(response.data)})
    let response = axios.get('http://localhost:3000/repeat/Bro').then(response => {repeatTxt.textContent = response.data})

    let repeatTxt = document.querySelector("#repeat-text")
...
const queryTest = () => {axios.get('http://localhost:3000/query-test?name=panda&status=bro').then(response => {console.log(response.data)})}
